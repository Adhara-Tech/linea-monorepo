...
import { IATCBridge } from "./interfaces/IATCBridge.sol";
import { IMessageService } from "../messaging/interfaces/IMessageService.sol";

import { BeaconProxy } from "@openzeppelin/contracts/proxy/beacon/BeaconProxy.sol";
import { ReentrancyGuardUpgradeable } from "@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol";

import { ATC } from "./ATC.sol";
import { BridgedATC } from "./BridgedATC.sol";
...
import { StorageFiller39 } from "../bridging/token/utils/StorageFiller39.sol";
 ...
 */
abstract contract ATCBridgeBase is
  IATCBridge,
  ...
  StorageFiller39
{
  ...
  bytes private constant METADATA_NAME = abi.encodeCall(ATC.name, ());
  bytes private constant METADATA_SYMBOL = abi.encodeCall(ATC.symbol, ());
  bytes private constant METADATA_DECIMALS = abi.encodeCall(ATC.decimals, ());
  ...
  function bridgeToken(
    ...
    _bridgeToken(_operationId, _token, _amount, _recipient);
  }
  ...
  function _bridgeToken(
    ...
    if (nativeToken != EMPTY) {
      BridgedATC(_token).destroy(_operationId,msg.sender, _amount, "");
      chainId = targetChainId;
    } else {
      // Token is native
      // L1 step 1 ATC Token
      _amount = _transferTokensToEscrow(_operationId, _token);
    ...
    messageService.sendMessage{ value: msg.value }(
      remoteSender,
      msg.value, // fees
      abi.encodeCall(IATCBridge.completeBridging, (_operationId, nativeToken, _amount, _recipient, chainId, tokenMetadata))
    );
   ...
   * @param _operationId The hold operation ID.
   * @param _token The address of the token to be bridged.
   * @return amount The real amount being transferred to the recipient.
   */
  function _transferTokensToEscrow(string calldata _operationId, address _token) internal virtual returns (uint256 amount) {
    ...
    uint256 balanceBefore = ATC(_token).balanceOf(escrowAddress);
    ATC(_token).executeHold(_operationId);
    amount = ATC(_token).balanceOf(escrowAddress) - balanceBefore;
  }
  ...
  {
    _completeBridging(_operationId, _nativeToken, _amount, _recipient, _chainId, _tokenMetadata);
  }
  ...
  function _completeBridging(
    ...
    if (nativeMappingValue == NATIVE_STATUS || nativeMappingValue == DEPLOYED_STATUS) {
      ...
      ATC(_nativeToken).transfer(_operationId, address(this), _recipient, _amount, "");
    } else {
      ...
      BridgedATC(bridgedToken).create(_recipient, _amount);
    }
  ...
  function confirmDeployment(address[] memory _tokens) external payable {
    ...
    messageService.sendMessage{ value: msg.value }(
      remoteSender,
      msg.value, // fees
      abi.encodeCall(IATCBridge.setDeployed, (_tokens))
    );
  ...
  function deployBridgedToken(
    ...
    BridgedATC(bridgedTokenAddress).initialize(name, symbol, decimals);
