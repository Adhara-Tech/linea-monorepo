...
### MESSAGING_SETTER_ROLE

```solidity
bytes32 MESSAGING_SETTER_ROLE
```

The role required to set the messaging contract address.
...
```solidity
function __LineaRollup_init(struct ILineaRollup.InitializationData _initializationData) internal virtual
```
...
```solidity
function submitBlobs(struct ILineaRollup.BlobSubmission[] _blobSubmissions, bytes32 _parentShnarf, bytes32 _finalBlobShnarf) external virtual
```
...
```solidity
function _submitBlobs(struct ILineaRollup.BlobSubmission[] _blobSubmissions, bytes32 _parentShnarf, bytes32 _finalBlobShnarf) internal virtual
```
...
```solidity
function submitDataAsCalldata(struct ILineaRollup.CompressedCalldataSubmission _submission, bytes32 _parentShnarf, bytes32 _expectedShnarf) external virtual
```
...
### _submitDataAsCalldata

```solidity
function _submitDataAsCalldata(struct ILineaRollup.CompressedCalldataSubmission _submission, bytes32 _parentShnarf, bytes32 _expectedShnarf) internal virtual
```
...
### setMessagingContractAddress

```solidity
function setMessagingContractAddress(address _newContractAddress) external
```

Set the contract to use for messaging storages.

_MESSAGING_SETTER_ROLE is required to execute._

### finalizeBlocks

```solidity
function finalizeBlocks(bytes _aggregatedProof, uint256 _proofType, struct ILineaRollup.FinalizationDataV3 _finalizationData) external virtual
```
...
```solidity
function _finalizeBlocks(struct ILineaRollup.FinalizationDataV3 _finalizationData, uint256 _lastFinalizedBlock) internal virtual returns (bytes32 finalShnarf)
```
