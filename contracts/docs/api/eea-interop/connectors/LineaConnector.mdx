...
```solidity
function hashMessage(address _from, address _to, uint256 _fee, uint256 _valueSent, uint256 _messageNumber, bytes _calldata) external pure returns (bytes32 messageHash)
```

Duplicated cause of calldata parameter
